...
function Edit( props ) {
	...
	const [ createForm, dispatchCreate ] = useReducer(
		ratingFormReducer,
		RATING_FORM_INIT
	);
	const [ editForm, dispatchEdit ] = useReducer(
		ratingFormReducer,
		RATING_FORM_INIT
	);
	const setCreateField = ( field, value ) =>
		dispatchCreate( { type: 'SET_FIELD', field, value } );
	...
	const {
		...
		( select ) => {
			...
			return {
				...
				isLoadingRating: ratingId
					? store.isLoadingRating( ratingId )
					: false,
	...
	const previewClass = ratingId && selectedRating ? ' shuriken-rating' : '';
	...
	const createNewRating = () => {
		...
		if ( createForm.mirrorOf ) {
			...
			requestData.scale = Math.max(
				scaleRange.min,
				Math.min( scaleRange.max, createForm.scale )
			);
	...
	const openEditModal = () => {
		...
		const isDisplayOnly =
			...
			displayOnlyValue === 'true' ||
			parseInt( displayOnlyValue, 10 ) === 1;
		dispatchEdit( {
			type: 'MERGE',
			payload: {
				name: selectedRating.name || '',
				mirrorOf: selectedRating.mirror_of
					? String( selectedRating.mirror_of )
					: '',
				parentId: selectedRating.parent_id
					? String( selectedRating.parent_id )
					: '',
	...
	const updateRatingFn = () => {
		...
		if ( editForm.mirrorOf ) {
			...
			requestData.scale = Math.max(
				scaleRange.min,
				Math.min( scaleRange.max, editForm.scale )
			);
	...
	const preview = selectedRating
		? renderRatingPreview( selectedRating, wp.element.createElement )
		: [ null, null ];

	// Loading state
	const loading =
		isLoadingRating ||
		( ratingId && ! selectedRating && ! error && ! storeError );
	...
	return wp.element.createElement(
		...
		wp.element.createElement(
			...
			wp.element.createElement(
				PanelBody,
				{
					title: __( 'Settings', 'shuriken-reviews' ),
					initialOpen: true,
				},
				wp.element.createElement( ComboboxControl, {
					...
					onChange: ( value ) => {
						setAttributes( {
							ratingId: value ? parseInt( value, 10 ) : 0,
						} );
					},
				...
				wp.element.createElement( TextControl, {
					...
					help: __(
						'Optional anchor ID for linking to this rating.',
						'shuriken-reviews'
					),
				} ),
				...
				wp.element.createElement( CheckboxControl, {
					...
					help: __(
						'Hide the rating name and description. Useful in Query Loop layouts.',
						'shuriken-reviews'
					),
				} )
			),
			...
			wp.element.createElement( PanelColorSettings, {
				...
				initialOpen: false,
				colorSettings: ( () => {
					const ratingType = selectedRating
		...
		isModalOpen &&
			wp.element.createElement(
				...
				wp.element.createElement( TextControl, {
					...
					help: __(
						'Enter a descriptive name for this rating.',
						'shuriken-reviews'
					),
				} ),
				wp.element.createElement( TextControl, {
					...
					onChange: ( value ) =>
						setCreateField( 'description', value ),
					placeholder: __(
				...
				wp.element.createElement( SelectControl, {
					...
					options: [
						{
							label: __( 'Not a Mirror', 'shuriken-reviews' ),
							value: '',
						},
					].concat(
					...
					help: __(
						'Mirrors share vote data with another rating.',
						'shuriken-reviews'
					),
				} ),
				! createForm.mirrorOf &&
				...
				! createForm.mirrorOf &&
					wp.element.createElement( SelectControl, {
						...
						options: [
							{
								label: __( 'None', 'shuriken-reviews' ),
								value: '',
							},
						].concat(
							parentRatings.map( ( r ) => {
						...
						onChange: ( value ) =>
							setCreateField( 'parentId', value ),
						help: __(
							'Sub-ratings contribute to parent score.',
							'shuriken-reviews'
						),
					} ),
				! createForm.mirrorOf &&
					...
					wp.element.createElement( SelectControl, {
						...
						options: [
							{
								label: __(
									'Positive — Higher votes improve parent rating',
									'shuriken-reviews'
								),
								value: 'positive',
							},
							{
								label: __(
									'Negative — Higher votes lower parent rating',
									'shuriken-reviews'
								),
								value: 'negative',
							},
						...
						onChange: ( value ) =>
							setCreateField( 'effectType', value ),
						help: __(
							'Negative is useful for aspects like "Difficulty" or "Price" where higher values are worse.',
							'shuriken-reviews'
						),
					} ),
				...
				! createForm.mirrorOf &&
					createForm.parentId &&
					( () => {
						const parent = parentRatings.find( ( r ) => {
							return String( r.id ) === createForm.parentId;
						} );
						if (
							parent &&
							! areTypesCompatible(
								parent.rating_type || 'stars',
								createForm.type
							)
						) {
							return wp.element.createElement(
								...
								__(
									"This sub-rating type is incompatible with the parent's type. Mixing star/numeric types with like/dislike/approval types produces incorrect aggregated scores.",
									'shuriken-reviews'
								)
							);
						}
				...
				! createForm.mirrorOf &&
					...
					wp.element.createElement( CheckboxControl, {
						...
						onChange: ( value ) =>
							setCreateField( 'displayOnly', value ),
						help: __(
							'No direct voting allowed.',
							'shuriken-reviews'
						),
					} ),
				wp.element.createElement(
			...
			wp.element.createElement(
				...
				! editForm.mirrorOf &&
					renderRatingTypeScaleFields( {
						...
						disabled:
							( parseInt(
								selectedRating && selectedRating.total_votes,
								10
							) || 0 ) > 0,
						typeHelp:
							( parseInt(
								...
								? __(
										'Type cannot be changed after votes have been cast.',
										'shuriken-reviews'
								  )
								: __(
										'Choose how users will rate this item.',
										'shuriken-reviews'
								  ),
						scaleHelp:
							( parseInt(
				...
				! editForm.mirrorOf &&
					...
					( () => {
						const parent = parentRatings.find( ( r ) => {
							return String( r.id ) === editForm.parentId;
						} );
						if (
							parent &&
							! areTypesCompatible(
				...
				! editForm.mirrorOf &&
					...
					wp.element.createElement( CheckboxControl, {
						...
						onChange: ( value ) =>
							setEditField( 'displayOnly', value ),
					} ),
				wp.element.createElement(
					'div',
					{
			...
			loading
				...
				? wp.element.createElement(
						...
						wp.element.createElement(
							...
							null,
							__(
								'Select a rating from the sidebar or create a new one.',
								'shuriken-reviews'
							)
						)
				  )
				...
				? wp.element.createElement(
						...
						! hideTitle &&
							selectedRating.label_description &&
							wp.element.createElement(
								'p',
