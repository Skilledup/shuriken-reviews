...
const formatApiError = ( err ) => {
	...
	if (
		...
		msg = __(
			'API endpoint not found. Please ensure the plugin is properly installed.',
			'shuriken-reviews'
		);
...
const makeErrorDismissers = ( setError, setLastFailedAction, clearError ) => {
	...
	return { retryLastAction, dismissError };
};
...
const useApiErrorHandling = ( clearError ) => {
	...
	const { retryLastAction: _retry, dismissError } = makeErrorDismissers(
		setError,
		setLastFailedAction,
		clearError
	);
	const retryLastAction = () => {
		_retry( lastFailedAction );
	};
...
const calculateAverage = ( rating ) => {
	...
	if ( tv > 0 ) {
		return (
			Math.round( ( parseFloat( rating.total_rating ) / tv ) * 100 ) / 100
		);
	}
	return 0;
};
...
const ratingTypeOptions = [
	...
	{
		label: __( 'Like / Dislike', 'shuriken-reviews' ),
		value: 'like_dislike',
	},
...
const renderRatingTypeScaleFields = ( opts ) => {
	...
	const derivedScaleHelp =
		scaleHelpMode === 'range'
			? `${ __( 'Scale range:', 'shuriken-reviews' ) }${ range.min }–${
					range.max
			  }`
			: type === 'stars'
 ...
 * @param {Function} createEl wp.element.createElement.
 * @return {Array} [widgetElement, statsElement]
 */
const renderRatingPreview = ( rating, createEl ) => {
	...
	if ( type === 'like_dislike' ) {
		...
		return [
			createEl(
				'div',
				{ className: 'shuriken-like-dislike display-only-stars' },
				h(
					'span',
					{ className: 'shuriken-btn shuriken-like-btn' },
					h(
						'span',
						{ className: 'shuriken-thumb' },
						iconThumbsUp( 18 )
					),
					h(
						'span',
						{ className: 'shuriken-count shuriken-like-count' },
						String( likes )
					)
				),
				h(
					'span',
					{ className: 'shuriken-btn shuriken-dislike-btn' },
					h(
						'span',
						{ className: 'shuriken-thumb' },
						iconThumbsDown( 18 )
					...
					h(
						'span',
						{ className: 'shuriken-count shuriken-dislike-count' },
						String( dislikes )
					)
				)
			),
	...
	if ( type === 'approval' ) {
		return [
			createEl(
				'div',
				{ className: 'shuriken-approval display-only-stars' },
				h(
					'span',
					{ className: 'shuriken-btn shuriken-upvote-btn' },
					h(
						'span',
						{ className: 'shuriken-thumb' },
						iconChevronUp( 14 )
					...
					h(
						'span',
						{ className: 'shuriken-count shuriken-upvote-count' },
						String( totalVotes )
					)
				)
			),
	...
	if ( type === 'numeric' ) {
		...
		const isDisplayOnly =
			rating.display_only === 1 ||
			rating.display_only === '1' ||
			rating.is_display_only === 1 ||
			rating.is_display_only === '1';
		const sliderVal =
			numAvg > 0 ? Math.round( numAvg ) : Math.round( scale / 2 );
		...
		if ( isDisplayOnly ) {
			...
			return [
				h(
					'div',
					{ className: 'shuriken-numeric display-only-stars' },
					h(
						'span',
						{ className: 'shuriken-numeric-display' },
						h(
							'span',
							{ className: 'shuriken-numeric-value' },
							String( displayVal )
						),
						h(
							'span',
							{ className: 'shuriken-slider-max' },
							`/ ${ scale }`
						)
					)
				),
		...
		return [
			createEl(
				...
				h(
					'span',
					{ className: 'shuriken-slider-value' },
					String( sliderVal )
				),
				h(
					'span',
...
const formatCompactStats = ( rating ) => {
	...
	if ( type === 'like_dislike' ) {
		return `+${ totalRating } / -${ Math.max(
			0,
			totalVotes - totalRating
		) }`;
	}
	if ( type === 'approval' ) {
	...
	return `${ scaledAvg }/${ scale } (${ totalVotes } ${ __(
		'votes',
		'shuriken-reviews'
	) })`;
};
