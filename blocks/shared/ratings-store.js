...
const scheduleBatchFetch = ( ratingId, args ) => {
	return new Promise( ( resolve, reject ) => {
		_batchQueue.push( { id: ratingId, resolve, reject } );
		if ( ! _batchTimer ) {
...
const thunks = {
	...
	searchRatings:
		...
		( args ) => {
			...
			const path = `/shuriken-reviews/v1/ratings/search?q=${ encodeURIComponent(
				searchTerm.trim()
		...
		return dedup( 'parents', () => {
			...
			return apiFetch( {
				...
				.catch( ( error ) => {
					...
					args.dispatch.setError(
						error.message || 'Failed to fetch parent ratings'
					);
					args.dispatch.setIsLoadingParents( false );
					return [];
				} );
		...
		return dedup( 'mirrorable', () => {
			...
			return apiFetch( {
				...
				.catch( ( error ) => {
					...
					args.dispatch.setError(
						error.message || 'Failed to fetch mirrorable ratings'
					);
					args.dispatch.setIsLoadingMirrorable( false );
					return [];
				} );
		...
		return dedup( `children-${ parentId }`, () => {
			return apiFetch( {
				...
				.catch( ( error ) => {
					...
					args.dispatch.setError(
						error.message || 'Failed to fetch child ratings'
					);
					return [];
				} );
		} );
	...
	fetchRatingsBatch: ( ids ) => ( args ) => {
		...
		return apiFetch( {
			...
			.catch( ( error ) => {
				...
				console.warn(
					'fetchRatingsBatch unavailable, falling back to individual fetches:',
	...
	fetchMirrorsForRating: ( ratingId ) => ( args ) => {
		...
		return dedup( `mirrors-${ ratingId }`, () => {
			...
			return apiFetch( {
				...
				.then( ( mirrors ) => {
					const mirrorsArray = Array.isArray( mirrors )
						? mirrors
						: [];
				...
				.catch( ( error ) => {
					...
					console.warn(
						`fetchMirrorsForRating failed for ${ ratingId }:`,
						error.code || error.message
					);
...
const reducer = ( state = DEFAULT_STATE, action ) => {
	switch ( action.type ) {
		...
		case ACTIONS.SET_RATINGS:
			...
			const ratingsArray = Array.isArray( action.ratings )
				? action.ratings
				: [];
		...
		case ACTIONS.SET_SEARCH_RESULTS:
			return {
				...state,
				searchResults: Array.isArray( action.results )
					? action.results
					: [],
			};
		...
		case ACTIONS.SET_PARENT_RATINGS:
			return {
				...state,
				parentRatings: Array.isArray( action.ratings )
					? action.ratings
					: [],
			};
		...
		case ACTIONS.SET_MIRRORABLE_RATINGS:
			return {
				...state,
				mirrorableRatings: Array.isArray( action.ratings )
					? action.ratings
					: [],
			};
		...
		case ACTIONS.ADD_TO_MIRRORABLE_RATINGS:
			return {
				...state,
				mirrorableRatings: [ action.rating ].concat(
					state.mirrorableRatings
				),
			};
		...
		case ACTIONS.UPDATE_RATING_IN_LISTS:
			...
			if ( ! rating.mirror_of ) {
				...
				const existsInMirrorable = state.mirrorableRatings.some(
					( r ) => {
						return (
							parseInt( r.id, 10 ) === parseInt( rating.id, 10 )
						);
					}
				);
				if ( existsInMirrorable ) {
					newMirrorableRatings = state.mirrorableRatings.map(
						( r ) => {
							return parseInt( r.id, 10 ) ===
								parseInt( rating.id, 10 )
								? rating
					...
					newMirrorableRatings = [ rating ].concat(
						state.mirrorableRatings
					);
				}
			} else {
				// Should not be in mirrorable list
				newMirrorableRatings = state.mirrorableRatings.filter(
					( r ) => {
						return (
							parseInt( r.id, 10 ) !== parseInt( rating.id, 10 )
						);
		...
		case ACTIONS.SET_MIRRORS_FOR_RATING:
			return {
				...
				mirrorsById: {
					...state.mirrorsById,
					[ action.ratingId ]: Array.isArray( action.mirrors )
						? action.mirrors
						: [],
				},
			};
...
const selectors = {
	...
	getMirrorableRatings( state ) {
		return Array.isArray( state.mirrorableRatings )
			? state.mirrorableRatings
			: [];
	},
	isSearching( state ) {
	...
	isLoadingMirrors( state, ratingId ) {
		return (
			( state.isLoadingMirrors && state.isLoadingMirrors[ ratingId ] ) ||
			false
		);
	},
};
