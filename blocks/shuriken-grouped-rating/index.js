...
function Edit( props ) {
	...
	const [ isManageChildrenModalOpen, setIsManageChildrenModalOpen ] =
		useState( false );
	...
	const [ parentForm, dispatchParent ] = useReducer(
		groupFormReducer,
		PARENT_FORM_INIT
	);
	const [ editParentForm, dispatchEditParent ] = useReducer(
		groupFormReducer,
		PARENT_FORM_INIT
	);
	const [ childForm, dispatchChild ] = useReducer(
		groupFormReducer,
		CHILD_FORM_INIT
	);
	const setParentField = ( field, value ) =>
		dispatchParent( { type: 'SET_FIELD', field, value } );
	const setEditParentField = ( field, value ) =>
		dispatchEditParent( { type: 'SET_FIELD', field, value } );
	const setChildField = ( field, value ) =>
		dispatchChild( { type: 'SET_FIELD', field, value } );
	...
	const [ creatingChildMirrorId, setCreatingChildMirrorId ] =
		useState( null );
	...
	const {
		...
		( select ) => {
			...
			return {
				...
				allRatingsById: store.getRatingsById
					? store.getRatingsById()
					: {},
				...
				parentMirrors: ratingId
					? store.getMirrorsForRating( ratingId )
					: null,
				parentMirrorsLoading: ratingId
					? store.isLoadingMirrors( ratingId )
					: false,
			};
		},
		[ ratingId ]
	...
	const previewClass =
		ratingId && selectedRating ? ' shuriken-rating-group' : '';
	...
	const loading =
		( isLoadingParents && parentRatings.length === 0 ) ||
		// Wait for parent mirror data (lift once mirror fetch finishes)
		( mirrorId &&
			! allRatingsById[ mirrorId ] &&
			( parentMirrors === null || parentMirrorsLoading ) ) ||
	...
	const _debouncedSearch = useSearchHandler(
		searchRatings,
		'parents_and_mirrors',
		20
	);
	...
	const childRatings = useMemo( () => {
		...
		Object.values( ratingsObj ).forEach( ( r ) => {
			if (
				r &&
				r.parent_id &&
				parseInt( r.parent_id, 10 ) === parseInt( ratingId, 10 )
			) {
				children.push( r );
			}
		} );
	...
	const childMirrorsMap = useSelect(
		( select ) => {
			const store = select( STORE_NAME );
			const map = {};
			childRatings.forEach( ( child ) => {
	...
	useEffect( () => {
		if ( ! ratingId || childRatings.length === 0 ) {
			return;
		}
		...
		if ( currentSubRatings.length === 0 ) {
			...
			if ( newEntries.length > 0 ) {
				setAttributes( {
					subRatings: currentSubRatings.concat( newEntries ),
				} );
			}
		}
	...
	const orderedVisibleChildren = useMemo( () => {
		...
		currentSubRatings.forEach( ( sr ) => {
			...
			const rating =
				allRatingsById[ displayId ] || allRatingsById[ sr.id ];
			if ( rating ) {
				result.push( rating );
			}
		} );
	...
	const createNewParentRating = () => {
		if ( ! parentForm.name.trim() || creating ) {
			return;
		}

		// Mirror creation mode
		if ( newRatingIsMirror ) {
			if ( ! newMirrorSourceId ) {
				return;
			}
			setCreating( true );
			createRating( {
				name: parentForm.name,
				mirror_of: newMirrorSourceId,
			} )
				.then( () => {
		...
		createRating( {
			...
			scale: Math.max(
				scaleRange.min,
				Math.min( scaleRange.max, parentForm.scale )
			),
			label_description: parentForm.description,
		} )
			.then( ( data ) => {
				setAttributes( {
					ratingId: parseInt( data.id, 10 ),
					mirrorId: 0,
					subRatings: [],
				} );
				fetchParentRatings();
	...
	const openEditParentModal = () => {
		...
		dispatchEditParent( {
			...
			payload: {
				...
				displayOnly:
					dv === true || dv === 'true' || parseInt( dv, 10 ) === 1,
	...
	const updateParentRating = () => {
		if ( ! editParentForm.name.trim() || updating || ! selectedRating ) {
			return;
		}
		setUpdating( true );
		...
		updateRating( selectedRating.id, {
			...
			scale: Math.max(
				scaleRange.min,
				Math.min( scaleRange.max, editParentForm.scale )
			),
	...
	const addNewChild = () => {
		if ( ! childForm.name.trim() || managingChildren || ! selectedRating ) {
			return;
		}
		...
		createRating( {
			...
			scale: Math.max(
				scaleRange.min,
				Math.min( scaleRange.max, childForm.scale )
			),
				...
				setChildrenToManage( ( prev ) => {
					...
					results.forEach( ( data ) => {
						const idx = updated.findIndex( ( r ) => {
							return (
								parseInt( r.id, 10 ) === parseInt( data.id, 10 )
							);
						} );
						if ( idx !== -1 ) {
							updated[ idx ] = data;
	...
	const deleteChild = ( childId ) => {
		if (
			! window.confirm(
				__(
					'Are you sure you want to delete this sub-rating?',
					'shuriken-reviews'
				)
			)
		) {
		...
		deleteRating( childId )
			.then( () => {
				setChildrenToManage( ( prev ) => {
					return prev.filter(
						( r ) =>
							parseInt( r.id, 10 ) !== parseInt( childId, 10 )
					);
				} );
				// Also remove from subRatings attribute
				const updated = ( subRatings || [] ).filter(
					( sr ) => sr.id !== parseInt( childId, 10 )
				);
				setAttributes( { subRatings: updated } );
				setError( null );
			} )
	...
	const createMirrorForParent = () => {
		if ( ! newMirrorName.trim() || creatingMirror || ! selectedRating ) {
			return;
		}
	...
	const deleteMirror = ( mirrorIdToDelete, sourceId ) => {
		...
		const retry = () => {
			deleteMirror( mirrorIdToDelete, sourceId );
		};
	...
	const createMirrorForChild = ( childId ) => {
		...
		if ( ! name || creatingChildMirrorId ) {
			return;
		}
		setCreatingChildMirrorId( parseInt( childId, 10 ) );
		createRating( { name, mirror_of: parseInt( childId, 10 ) } )
			.then( () => {
	...
	const ratingOptions = useMemo( () => {
		...
		if ( mirrorId && allRatingsById[ mirrorId ] ) {
			...
			if ( ! seen[ mRating.id ] ) {
				...
				options.push( {
					label: `${ mRating.name } — ${ __(
						'Mirror',
						'shuriken-reviews'
					) } (ID: ${ mRating.id })`,
					value: String( mRating.id ),
				} );
			}
		}
		...
		if ( searchTerm && searchTerm.trim().length > 0 ) {
			( Array.isArray( searchResults ) ? searchResults : [] ).forEach(
				( r ) => {
					if ( r && r.id && ! seen[ r.id ] ) {
						seen[ r.id ] = true;
	...
	const mirrorableOptions = useMemo( () => {
		const options = [];
		( Array.isArray( mirrorableRatings ) ? mirrorableRatings : [] ).forEach(
			( r ) => {
				if ( r && r.id ) {
					let label = `${ r.name } (ID: ${ r.id })`;
					if ( r.parent_id ) {
					...
					options.push( { label, value: String( r.id ) } );
				}
			}
		);
		return options;
	...
	const modalCtx = {
		...
		mirrorableOptions,
		isLoadingMirrorable,
		newParentName: parentForm.name,
		setNewParentName: ( v ) => setParentField( 'name', v ),
		handleCreateKeyDown,
		newParentDescription: parentForm.description,
		setNewParentDescription: ( v ) => setParentField( 'description', v ),
		newParentDisplayOnly: parentForm.displayOnly,
		setNewParentDisplayOnly: ( v ) => setParentField( 'displayOnly', v ),
		newParentType: parentForm.type,
		setNewParentType: ( v ) => setParentField( 'type', v ),
		newParentScale: parentForm.scale,
		setNewParentScale: ( v ) => setParentField( 'scale', v ),
		createNewParentRating,
		creating,
		// Edit-parent modal
		isEditModalOpen,
		setIsEditModalOpen,
		editParentName: editParentForm.name,
		setEditParentName: ( v ) => setEditParentField( 'name', v ),
		handleEditKeyDown,
		editParentDescription: editParentForm.description,
		setEditParentDescription: ( v ) =>
			setEditParentField( 'description', v ),
		editParentDisplayOnly: editParentForm.displayOnly,
		setEditParentDisplayOnly: ( v ) =>
			setEditParentField( 'displayOnly', v ),
		editParentType: editParentForm.type,
		setEditParentType: ( v ) => setEditParentField( 'type', v ),
		editParentScale: editParentForm.scale,
		setEditParentScale: ( v ) => setEditParentField( 'scale', v ),
		selectedRating,
		updateParentRating,
		updating,
		parentMirrors,
		...
		isManageChildrenModalOpen,
		setIsManageChildrenModalOpen,
		childrenLocalEdits,
		setChildrenLocalEdits,
		newChildName: childForm.name,
		setNewChildName: ( v ) => setChildField( 'name', v ),
		handleChildKeyDown,
		newChildDescription: childForm.description,
		setNewChildDescription: ( v ) => setChildField( 'description', v ),
		newChildEffectType: childForm.effectType,
		setNewChildEffectType: ( v ) => setChildField( 'effectType', v ),
		newChildType: childForm.type,
		setNewChildType: ( v ) => setChildField( 'type', v ),
		newChildScale: childForm.scale,
		setNewChildScale: ( v ) => setChildField( 'scale', v ),
		newChildDisplayOnly: childForm.displayOnly,
		setNewChildDisplayOnly: ( v ) => setChildField( 'displayOnly', v ),
		addNewChild,
		managingChildren,
		childrenToManage,
		deleteChild,
		...
		creatingChildMirrorId,
		createMirrorForChild,
		applyChildrenEdits,
		savingChildren,
	};
			...
			loading
				...
				? wp.element.createElement(
						...
						wp.element.createElement(
							...
							null,
							__(
								'Select a parent rating to display the group.',
								'shuriken-reviews'
							)
						)
				  )
				...
				? wp.element.createElement(
						...
						wp.element.createElement(
							'div',
							{
								className: `shuriken-rating parent-rating${
									parentDisplayRating.display_only
										? ' display-only'
										: ''
								}`,
							},
							wp.element.createElement(
								...
								! hideTitle &&
									parentDisplayRating.label_description &&
									wp.element.createElement(
										'p',
						...
						orderedVisibleChildren.length > 0 &&
							wp.element.createElement(
								'div',
								...
								orderedVisibleChildren.map( ( child ) => {
									return wp.element.createElement(
										'div',
										{
											key: child.id,
											className:
												'shuriken-rating child-rating',
										},
										wp.element.createElement(
											'div',
											{
												className:
													'shuriken-rating-wrapper',
											},
											! hideTitle &&
												wp.element.createElement(
													'h4',
				...
				: wp.element.createElement(
						...
						wp.element.createElement(
							...
							null,
							__(
								'Rating not found. It may have been deleted.',
								'shuriken-reviews'
							)
						)
				  )
...
if ( typeof window.wp?.hooks?.applyFilters === 'function' ) {
	settings = window.wp.hooks.applyFilters(
		'shurikenBlockSettings_groupedRating',
		settings
	);
}
