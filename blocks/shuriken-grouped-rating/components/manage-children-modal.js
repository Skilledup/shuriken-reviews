...
export default function ManageChildrenModal( ctx ) {
	...
	const {
		...
		setNewChildName,
		setNewChildEffectType,
		setNewChildType,
		setNewChildScale,
		setNewChildDisplayOnly,
	...
	return wp.element.createElement(
		...
		selectedRating &&
			wp.element.createElement(
				...
				Object.keys( childrenLocalEdits ).length > 0 &&
					wp.element.createElement(
						'span',
						{ className: 'shuriken-badge is-info' },
						`${ Object.keys( childrenLocalEdits ).length } ${ __(
							'unsaved',
							'shuriken-reviews'
						) }`
					)
		...
		wp.element.createElement(
			...
			wp.element.createElement(
				...
				wp.element.createElement(
					'h4',
					null,
					__( 'Add New Sub-Rating', 'shuriken-reviews' )
				)
			),
			wp.element.createElement( TextControl, {
				...
				placeholder: __(
					'e.g., Build Quality, Features, Value for Money',
					'shuriken-reviews'
				),
			} ),
			wp.element.createElement( TextControl, {
			...
			wp.element.createElement( CheckboxControl, {
				...
				help: __(
					'When enabled, this sub-rating cannot be voted on directly.',
					'shuriken-reviews'
				),
			} ),
			wp.element.createElement(
		...
		wp.element.createElement(
			...
			wp.element.createElement(
				...
				wp.element.createElement(
					'h4',
					null,
					__( 'Existing Sub-Ratings', 'shuriken-reviews' )
				),
				wp.element.createElement(
					'span',
					{ className: 'shuriken-badge' },
					childrenToManage.length
				)
			),
			childrenToManage.length === 0 &&
				wp.element.createElement(
					'p',
					{ className: 'shuriken-empty-message' },
					__(
						'No sub-ratings yet. Add one above.',
						'shuriken-reviews'
					)
				),
			childrenToManage.map( ( child ) => {
				...
				return wp.element.createElement(
					...
					{
						key: child.id,
						className: `shuriken-child-card${
							hasEdits ? ' is-modified' : ''
						}`,
					},
					// Child header row
					wp.element.createElement(
					...
					wp.element.createElement(
						...
						wp.element.createElement( TextControl, {
							...
							value:
								( hasEdits &&
									childrenLocalEdits[ child.id ].name ) ||
								child.name,
							onChange: ( value ) => {
						...
						( () => {
							...
							const childScale =
								hasEdits &&
								childrenLocalEdits[ child.id ].scale !==
									undefined
									? childrenLocalEdits[ child.id ].scale
									: child.scale || 5;
							const childDisplayOnly =
								hasEdits &&
								childrenLocalEdits[ child.id ].display_only !==
									undefined
									? childrenLocalEdits[ child.id ]
											.display_only
									: child.display_only || false;
							const childHasVotes =
								parseInt( child.total_votes, 10 ) > 0;
							return wp.element.createElement(
								wp.element.Fragment,
								null,
								renderRatingTypeScaleFields( {
								...
								selectedRating &&
									...
									wp.element.createElement(
										...
										__(
											"This sub-rating type is incompatible with the parent's type. Aggregated scores may be incorrect.",
											'shuriken-reviews'
										)
									)
							);
					...
					( () => {
						...
						return wp.element.createElement(
							...
							cHasMirrors &&
								cMirrors.map( ( cm ) => {
									const isEditing =
										editingMirrorNames.hasOwnProperty(
											cm.id
										);
									const isSaving =
										savingMirrorId ===
									...
									if ( isEditing ) {
										return wp.element.createElement(
											'div',
											{
												key: cm.id,
												className:
													'shuriken-mirror-card is-editing',
											},
											wp.element.createElement(
												TextControl,
												{
													value: editingMirrorNames[
														cm.id
													],
													onChange: ( val ) => {
														updateEditingMirrorName(
									...
									return wp.element.createElement(
										...
										{
											key: cm.id,
											className: 'shuriken-mirror-card',
										},
										wp.element.createElement(
											'span',
											{
												className:
													'shuriken-mirror-name',
											},
											cm.name,
											wp.element.createElement(
												'span',
			...
			Object.keys( childrenLocalEdits ).length > 0 &&
				wp.element.createElement(
					Notice,
					{
						status: 'info',
						isDismissible: false,
						className: 'shuriken-unsaved-notice',
					},
					__(
