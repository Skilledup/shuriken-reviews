	...
	const ShurikenPostSidebar = () => {
		...
		useEffect( () => {
			...
			apiFetch( {
				...
				.catch( ( err ) => {
					setError(
						err.message ||
							__( 'Failed to load', 'shuriken-reviews' )
					);
					setLoading( false );
				} );
		}, [ postId, postType ] );
		...
		return el(
			...
			loading
				? el(
						'div',
						{ className: 'shuriken-sidebar-loading' },
						el( Spinner ),
						' ',
				...
				: el(
						'div',
						{ className: 'shuriken-sidebar-list' },
						data.map( ( item ) => {
							return el(
								'div',
								{
									key: item.id,
									className: 'shuriken-sidebar-item',
								},
								el(
									'div',
									{ className: 'shuriken-sidebar-item-name' },
									item.name
								),
								el(
									'div',
